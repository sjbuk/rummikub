// POST /rooms-create { name, isPublic?, preset? } -> 200 PublicState
import { serve } from 'https://deno.land/std@0.224.0/http/server.ts';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2.44.4';
import { handleOptions, jsonWithCors } from '../_shared/cors.ts';
import { handleRoomsCreate } from '../_shared/server/handlers.ts';
...
serve(async (req: Request) => {
  ...
  const res = await handleRoomsCreate(db, input);
